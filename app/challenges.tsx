...
export default function Challenges() {
    ...
    onValue(usersRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const usersArray = Object.entries(data).map(([uid, user]: any) => ({
          ...
          completedCount:
            Object.values(user?.acceptedDares || {}).filter((d: any) => d.status === "completed").length || 0,
        }))
  ...
  const handleLikeDare = async (dareId: string, likedBy: string[] = []) => {
    try {
      ...
      const newLiked = isLiking ? [...likedBy, user.uid] : likedBy.filter((u) => u !== user.uid)
  ...
  const handleUploadEvidence = async (dareId) => {
    try {
      ...
      if (ai.isCompleted) {
        ...
        } else if (ai.description.toLowerCase().includes("object") || ai.tags.includes("indoor")) {
        ...
        Alert.alert("Evidence Rejected", reason, [
          {
            text: "Retry",
      ...
      if (dareSnapshot.exists()) {
        ...
        const completedByUser = Object.values(allDares).filter(
          (d: any) => d.status === "completed" && d.acceptedBy?.[user.uid],
        )
          ...
          >
            ...
            {item.userId !== user?.uid && (
              ...
              >
                ...
                <Text style={{ color: "#fff", fontFamily: "Montserrat-SemiBold", fontSize: 12 }}>View Profile</Text>
              </TouchableOpacity>
            )}
          </View>
      ...
      <LinearGradient colors={["#4B0082", "#B788C4"]} style={styles.gradient}>
        ...
        <View style={styles.container}>
          <View style={styles.headerContainer}>
            ...
            {badges.length > 0 && (
              <Text style={{ color: "#fff", fontStyle: "italic", marginTop: 5 }}>🏅 Badges: {badges.join(", ")}</Text>
            )}
                ...
                selectedEvidence && (
                  <WebView
                    ...
                    onError={(e) => {
                      ...
                      Alert.alert("Error", "Failed to load the image. It may have expired or is unavailable.")
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              ...
              <TouchableOpacity style={styles.menuButton} onPress={() => setTimedOutModalVisible(false)}>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContainer, { maxHeight: 400 }]}>
              ...
              <TouchableOpacity style={styles.menuButton} onPress={() => setLikeModalVisible(false)}>
                <Text style={styles.menuButtonText}>Close</Text>
              </TouchableOpacity>
            </View>
          </View>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              ...
              <Text style={[styles.commentText, { marginBottom: 15 }]}>Are you sure you want to delete this dare?</Text>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={[styles.commentText, { marginBottom: 15 }]}>Are you sure you want to accept this dare?</Text>
            <TouchableOpacity
              style={styles.menuButton}
              onPress={async () => {
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={[styles.commentText, { marginBottom: 15 }]}>Are you sure you want to decline this dare?</Text>
            <TouchableOpacity
              style={styles.menuButton}
              onPress={async () => {
